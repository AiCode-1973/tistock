...
// Sugere o próximo código automático
$ultimoCodigo = $pdo->query(
    "SELECT codigo FROM kb_pops ORDER BY id DESC LIMIT 1"
)->fetchColumn();

$proximoCodigo = 'POP-TI-001';
if ($ultimoCodigo && preg_match('/(\d+)$/', $ultimoCodigo, $m)) {
    $proximoCodigo = preg_replace('/\d+$/', str_pad((int)$m[1] + 1, 3, '0', STR_PAD_LEFT), $ultimoCodigo);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        $chk = $pdo->prepare("SELECT id FROM kb_pops WHERE codigo = ? LIMIT 1");
        $chk->execute([$codigo]);
        if ($chk->fetch()) $erros[] = "Já existe um POP com o código \"{$codigo}\".";
    }
        ...
        $pdo->prepare(
            "INSERT INTO kb_pops
             (codigo, titulo, versao, data_elaboracao, objetivo, escopo,
              responsavel_execucao, responsavel_elaboracao, procedimento, referencias, status, autor_id)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )->execute([
            $codigo, $titulo, $versao, $dataElaboracao, $objetivo, $escopo,
            $respExecucao, $respElaboracao, $procedimento,
            $referencias ?: null, $status, $_SESSION['usuario_id'],
        ]);

        $novoId = (int) $pdo->lastInsertId();
        registrarLog($pdo, 'POP_CRIAR', "POP criado: {$codigo} \"{$titulo}\" (ID {$novoId})");
        setFlash('success', "POP \"{$codigo}\" criado com sucesso!");
        header('Location: ' . BASE_URL . '/pages/conhecimento/pops/visualizar.php?id=' . $novoId);
        exit;
    }
}
...
<div class="d-flex align-items-center mb-4">
    ...
    <h4 class="fw-bold mb-0"><i class="fas fa-plus-circle me-2 text-primary"></i>Novo POP</h4>
</div>
        ...
        <div class="col-lg-8">
                    ...
                    <div class="row g-3">
                        <div class="col-sm-4">
                            ...
                            <input type="text" id="codigo" name="codigo" class="form-control text-uppercase"
                                   ...
                                   value="<?= htmlspecialchars($_POST['codigo'] ?? $proximoCodigo, ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                        <div class="col-sm-6">
                        ...
                        <div class="col-sm-2">
                            ...
                            <input type="text" id="versao" name="versao" class="form-control"
                                   maxlength="10"
                                   value="<?= htmlspecialchars($_POST['versao'] ?? '1.0', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                        <div class="col-sm-3">
                            ...
                            <input type="date" id="data_elaboracao" name="data_elaboracao" class="form-control"
                                   value="<?= htmlspecialchars($_POST['data_elaboracao'] ?? date('Y-m-d'), ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                        <div class="col-sm-5">
                            ...
                            <input type="text" id="responsavel_elaboracao" name="responsavel_elaboracao"
                                   ...
                                   value="<?= htmlspecialchars($_POST['responsavel_elaboracao'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                        <div class="col-sm-6">
                            ...
                            <input type="text" id="responsavel_execucao" name="responsavel_execucao"
                                   class="form-control" maxlength="200" required
                                   value="<?= htmlspecialchars($_POST['responsavel_execucao'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        ><?= htmlspecialchars($_POST['objetivo'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>
                    <div class="mb-0">
                        ...
                        ><?= htmlspecialchars($_POST['escopo'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    ...
                    ><?= htmlspecialchars($_POST['referencias'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    <div class="mb-4">
                        ...
                        <select id="status" name="status" class="form-select">
                            ...
                            <option value="revisao"  <?= ($_POST['status'] ?? '') === 'revisao'  ? 'selected' : '' ?>>Em Revisão</option>
                            <option value="obsoleto" <?= ($_POST['status'] ?? '') === 'obsoleto' ? 'selected' : '' ?>>Obsoleto</option>
                        </select>
                    </div>
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-1"></i>Salvar POP
                        </button>
...
<?php if (!empty($_POST['procedimento'])): ?>
quill.root.innerHTML = <?= json_encode($_POST['procedimento']) ?>;
<?php endif; ?>
...
// Força código para maiúsculas enquanto digita
