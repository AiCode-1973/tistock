<?php
/**
 * TI Stock - Base de Conhecimento - Editar Categoria KB
 */
...
$stmt = $pdo->prepare("SELECT * FROM kb_categorias WHERE id = ? LIMIT 1");
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        $stmt2 = $pdo->prepare("SELECT id FROM kb_categorias WHERE nome = ? AND id != ? LIMIT 1");
        $stmt2->execute([$nome, $id]);
        if ($stmt2->fetch()) {
            $erros[] = 'Já existe outra categoria KB com este nome.';
        }
    }

    if (empty($erros)) {
        $pdo->prepare("UPDATE kb_categorias SET nome = ?, descricao = ? WHERE id = ?")
...
<div class="d-flex align-items-center mb-4">
    <a href="<?= BASE_URL ?>/pages/conhecimento/categorias/listar.php" class="btn btn-outline-secondary btn-sm me-3">
        <i class="fas fa-arrow-left"></i>
    </a>
    <h4 class="fw-bold mb-0"><i class="fas fa-edit me-2 text-primary"></i>Editar Categoria KB</h4>
</div>
            ...
            <div class="card-body p-4">
                <form method="POST" action="" novalidate>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="<?= BASE_URL ?>/pages/conhecimento/categorias/listar.php" class="btn btn-outline-secondary">Cancelar</a>
                    </div>
