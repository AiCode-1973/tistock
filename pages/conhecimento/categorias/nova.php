<?php
/**
 * TI Stock - Base de Conhecimento - Nova Categoria KB
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        $stmt = $pdo->prepare("SELECT id FROM kb_categorias WHERE nome = ? LIMIT 1");
        $stmt->execute([$nome]);
        if ($stmt->fetch()) {
            $erros[] = 'Já existe uma categoria KB com este nome.';
        }
    }

    if (empty($erros)) {
        $pdo->prepare("INSERT INTO kb_categorias (nome, descricao) VALUES (?, ?)")
...
<div class="d-flex align-items-center mb-4">
    ...
    <h4 class="fw-bold mb-0"><i class="fas fa-plus-circle me-2 text-primary"></i>Nova Categoria KB</h4>
</div>
